...
// \param i32NumParams is the number of elements in the pcParam array.
//
...
//*****************************************************************************
int32_t
FindCGIParameter(const char *pcToFind, char *pcParam[], int32_t i32NumParams)
{
    int32_t i32Loop;
    ...
    //
    for(i32Loop = 0; i32Loop < i32NumParams; i32Loop++)
    {
        //
        // Does the parameter name match the provided string?
        //
        if(strcmp(pcToFind, pcParam[i32Loop]) == 0)
        {
            //
...
//*****************************************************************************
uint32_t
EncodeFormString(const char *pcDecoded, char *pcEncoded, uint32_t ui32Len)
{
    uint32_t ui32Loop;
    uint32_t ui32Count;
...
//*****************************************************************************
uint32_t
DecodeFormString(const  char *pcEncoded, char *pcDecoded, uint32_t ui32Len)
{
    uint32_t ui32Loop;
    uint32_t ui32Count;
    bool bValid;
...
CheckDecimalParam(const char *pcValue, int32_t *pi32Value)
{
    ...
    while(pcValue[ui32Loop])
    {
        ...
        if(bStarted)
        {
            if(!bFinished)
            {
                ...
                {
                    l32Accum = (l32Accum * 10) + (pcValue[ui32Loop] - '0');
                }
                else
                {
                    //
    ...
    //
    *pi32Value =  bNeg ? -l32Accum : l32Accum;
    return(true);
}
            ...
            int32_t i32NumParams, bool *pbError)
{
    ...
    //
    i32Param = FindCGIParameter(pcName, pcParams, i32NumParams);
    if(i32Param != -1)
    {
        //
